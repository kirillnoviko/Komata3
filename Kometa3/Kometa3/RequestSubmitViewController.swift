...
class RequestSubmitViewController: BaseViewControllerMainButton , UITextFieldDelegate{
    ...
    @objc private func keyboardWillShow(_ notification: Notification) {
        ...
        // Поднимаем представление на высоту клавиатуры
        adjustViewForKeyboard(isShowing: true, keyboardHeight: keyboardHeight)
    }

    @objc private func keyboardWillHide(_ notification: Notification) {
        // Возвращаем представление в исходное состояние
        adjustViewForKeyboard(isShowing: false, keyboardHeight: 0)
    }

    private func adjustViewForKeyboard(isShowing: Bool, keyboardHeight: CGFloat) {
        UIView.animate(withDuration: 0.3) {
            if isShowing {
                self.view.frame.origin.y = -keyboardHeight / 2 // Или любое значение, которое вам подходит
            } else {
